...
describe "Ruby Encrypt/Decrypt test vs Java reference impl" do
  ...
  let(:test_jks_keystore) { fixture_path("test_server.jks") }
  let(:test_pc12_server_key) { fixture_path("test_server.p12") }
  let(:test_pc12_client_key) { fixture_path("test_client.p12") }
  let(:test_keystore_pass) { "importkey" }

  it "encrypts in java, and decrypts using ruby" do
    p12 = OpenSSL::PKCS12.new(File.read(test_pc12_client_key), test_keystore_pass)

    encrypted_xml = VBMS.encrypted_soap_document(
      plaintext_xml, test_jks_keystore, test_keystore_pass, plaintext_request_name)
    decrypted_xml = VBMS.decrypt_message_xml_ruby(encrypted_xml, p12.key,
                                                  test_keystore_pass)
